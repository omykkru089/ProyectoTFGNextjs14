...
async function fetchDataByLink(link: string): Promise<juego | null> {
  try {
    // Hacemos un fetch al backend para obtener el juego usando el link
    ...
    return {
      id: data.id,
      imagen_de_portada: data.imagen_de_portada, // Mapeamos 'imagen_de_portada' a 'img'
      nombre: data.nombre,
      link: data.link,
      precio: data.precio,
      descripcion: data.descripcion,
      categoria: data.categoria,
      fecha_de_lanzamiento: data.fecha_de_lanzamiento, // Mapeamos 'fecha_de_lanzamiento' a 'fechaLanzamiento'
      desarrollador: data.desarrolladora, // Mapeamos 'desarrolladora' a 'desarrollador'
...
export default async function Page({ params }: { readonly params: Params }) {
  ...
  // Obtenemos los datos del juego usando el link
  const juego = await fetchDataByLink(link);

  // Si no se encuentra el juego, renderiza la página de error 404
  if (!juego) {
    return (
  ...
  // Renderiza los detalles del juego si se encuentra
  return (
    <>
    <Header />
  ...
  title={juego.nombre}
></iframe>
            <Image id="img-1" width={700} height={700} src={juego.imagen_de_portada} className='w-[300px] h-[300px] flex-[1_0_100%] object-cover [scroll-snap-align:start]' alt={juego.nombre}></Image>
          </div>
      ...
      <article>
        <div className='w-[400px] h-[250px] bg-[#ece4f92c] rounded-[2rem] [box-shadow:0px_0px_10px_#B99DE7] relative'>
              <div className=' relative grid place-content-center font-extrabold text-3xl text-[#fff] pt-2 [filter:drop-shadow(0px_0px_10px_#50307C)] '>{juego.nombre}</div>
                ...
                <span className='[filter:drop-shadow(0px_0px_2px_#A167D8)] '>{juego.plataforma.nombre}</span> 
                <img src="/check.png" alt="check" className='w-2.5 h-2.5 mt-1.5 mr-[-4px] [filter:drop-shadow(0px_0px_2px_green)]'/>
                ...
                <ul className="flex items-center space-x-6 w-[120px] bg-[#ddbbf770] p-2 rounded-t-md">
                  ...
                  <li className="relative group">
                    ...
                    <ul className="absolute z-20 left-[-7.5%] w-[135.5px] hidden bg-[#ddbbf770] rounded-b-md shadow-lg group-hover:block">
                      <li><Link href={juego.link[2]} className="block px-4 py-2 hover:bg-[#ddbbf790]">Xbox</Link></li>
                      <li><Link href={juego.link[3]} className="block px-4 py-2 hover:bg-[#ddbbf790]">Nintendo</Link></li>
                      <li><Link href={juego.link[1]} className="block px-4 py-2 hover:bg-[#ddbbf790] rounded-b-md">PlayStation</Link></li>
                    </ul>
                  </li>
                </ul>
                <ul className="flex items-center space-x-6 w-[120px] bg-[#ddbbf770] p-2 rounded-t-md">
          ...
          <div className=' relative flex justify-center text-[#19ff38] text-3xl font-bold top-4 mb-[15px] border-[1px] rounded-lg pb-[2px] border-green-300 w-[85px] left-[40%] '><span className='[filter:drop-shadow(0px_0px_4px_green)]'>60€</span></div>
          <div className='grid place-content-center relative top-[14%]'><button className="w-80 text-[#fff] cursor-pointer border-[1px] border-[solid] border-[black] rounded-[4px] px-[0.3em] py-[0.3em] bg-[black] [transition:0.2s] mt-[-15px] hover:-translate-x-[0] hover:-translate-y-1 hover:bg-[#DDBBF7] hover:[box-shadow:0_0.25rem_#000] active:translate-x-[0] active:[box-shadow:none]">Comprar Ahora!</button></div>
        </div>  
      </article>
    </section>
    <section className=' w-[100%] mb-10 '>
      <div className='border-t-[3px] border-[#50307c60] mt-[40px] w-[80%] ml-[10%] rounded-full'></div>
        <article className='ml-[10%] pt-5 '>
          ...
          <p className='w-[80%] text-[#6b6b6b] pl-[20px]'>{juego.descripcion}</p> 
        </article>
    </section >
...
></iframe>
        ...
        {Array.isArray(juego.imagen_de_portada) && juego.imagen_de_portada.length > 0 ? (
    juego.imagen_de_portada.map((imagen, index) => (
      <Image
        key={`${imagen}-${index}`} // Clave única combinando la URL y el índice
        width={700}
        height={700}
        src={imagen}
    ...
    <p className="text-[#fff]">No hay imágenes disponibles</p>
  )}
</div>
      </article>
    </section>
  ...
  {Array.isArray(juego.requisitos_del_sistema) && juego.requisitos_del_sistema.length > 0 ? (
    juego.requisitos_del_sistema.map((requisito, index) => (
      <li key={`${requisito}-${index}`}>{requisito}</li> // Clave única combinando el texto y el índice
    ))
  ) : (
